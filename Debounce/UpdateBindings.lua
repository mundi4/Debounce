    ...
    for state, stateInfo in pairs(_customStates) do
        if (stateInfo) then
            ...
            if (stateInfo.value ~= nil) then
                ...
                appendLine([[self:RunAttribute("SetCustomState", %1$q, %s)]], state, tostring(stateInfo.value));
            end
...
-- local arr = { "hover", "target", "player", "mouseover" };


-- sort(arr, function(l,r)
--     if (l == "mouseover") then
--         return true;
...
-- dump("hello2", MacroConditionals["[@custom1,exists,$zzz]hello"]);
-- dump("hello3", MacroConditionals["[@custom1,exists,$zzz]hello"]);
-- wipe(MacroConditionals);
-- dump("hello", MacroConditionals["[@custom1,exists,$zzz]hello"]);
-- dump("hello2", MacroConditionals["[@custom1,exists,$zzz]hello"]);
...
function UpdateBindingsMap()
    ...
    for key, bindingArray in pairs(DebouncePrivate.KeyMap) do
        ...
        -- 하나의 매크로로 조합: 상태 변경을 감지하지 않아도 된다.
        -- 일부 조건(mouseover 등)은 상태 변경을 즉각적으로 감지할 수 없기 때문에 상태 변경 감지에 의존하는 건 딜레이가 생길 수 있다.(기본 wow 코드 상으로는 최대 0.2초)
        -- 하나의 매크로로 조합하게 되면 조건 체크가 단축키를 누르는 순간 이루어질 것이기 때문에 위의 문제가 사라진다.
        -- 애드온이 하는 일이 줄어들기 때문에 성능 상의 이점도 있을 것?
        -- 매크로로 조합할 수 없는 경우
        --      UNUSED, COMMAND 등 클릭으로 바인딩할 수 없는 경우: 매크로로 이 행동을 실행할 수 있는 방법이 없다.
        -- 		하나 이상의 유닛을 체크하는 경우: 두 개 이상의 유닛을 동시에 체크할 수 없다. (예외: pet은 항상 체크 가능)
        --      hover 조건에 frameTypes을 사용하는 경우.
        -- 		specialbar == true: 가능은 하지만 매크로가 몇 배로 복잡해진다! 나중에 구현해보자. [...,overridebar][...,vehicleui][...,possessbar][...,shapeshift][...,petbattle]
        --      모든 바인딩이 conditional인 경우. 조건이 맞지 않을 경우 단축키를 해제해야하므로 매크로만으로는 불가능.
        -- local combinedClick;
        -- local combinableClicksHelp;
        -- local combinableClicksHarm;
...
self:SetAttribute("state-unitexists", 0)
    ...
    if (_unitStates.mouseover) then
        appendLine([[stateValue=UnitExists("mouseover")]]);
        appendLine([[if (UnitStates.mouseover ~= stateValue) then UnitStates.mouseover=stateValue;DirtyFlags["mouseover-exists"]=true; end]]);
    end
    ...
    for state, stateInfo in pairs(_customStates) do
        if (stateInfo) then
            if (stateInfo.mode == CUSTOM_STATE_MODES.MACRO_CONDITIONAL) then
                ...
                appendLine([[if (States[%1$q] ~= stateValue) then self:RunAttribute("SetCustomState", %1$q, stateValue) end]], stateInfo.name);
            end
        end
    end
